...
function Install_ct() {
  ...
  wget --no-check-certificate https://gitee.com/qq348069510/gost/raw/master/download/v"$ct_new_ver"/gost-linux-"$bit"-"$ct_new_ver".gz
  gunzip gost-linux-"$bit"-"$ct_new_ver".gz
  mv gost-linux-"$bit"-"$ct_new_ver" gost
  mv gost /usr/bin/gost
  chmod -R 777 /usr/bin/gost
  wget --no-check-certificate https://gitee.com/qq348069510/Multi-EasyGost/raw/master/gost.service && chmod -R 777 gost.service && mv gost.service /usr/lib/systemd/system
  mkdir /etc/gost && wget --no-check-certificate https://gitee.com/qq348069510/Multi-EasyGost/raw/master/config.json && mv config.json /etc/gost && chmod -R 777 /etc/gost
